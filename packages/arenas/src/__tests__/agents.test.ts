...
import { resolveBaselines } from "@solclash/agents";

describe("resolveBaselines", () => {
  test("resolves known baselines", () => {
    const result = resolveBaselines(["BUY_AND_HOLD", "FLAT"]);
  ...
  test("collects unknown baselines without throwing", () => {
    const result = resolveBaselines(["BUY_AND_HOLD", "UNKNOWN_BASELINE"]);
